...
<center>The DBMS must include concurrency control routines to ensure that several users trying to update the same data do so in a controlled manner => support for online transaction processing (OLTP).</center>
